...
test.describe('Accessibility Tests', () => {
  ...
  test('Heading hierarchy - home page has correct h1 → h2 → h3 structure', async ({ page }) => {
    await page.goto('/');
    await page.waitForSelector('[data-testid="games-grid"]', { timeout: 10000 });

    await test.step('Page has exactly one h1', async () => {
      const h1s = page.locator('h1');
      await expect(h1s).toHaveCount(1);
      await expect(h1s.first()).toContainText('Welcome to Tailspin Toys');
    });

    await test.step('h2 follows h1 for section heading', async () => {
      const h2s = page.locator('h2');
      await expect(h2s.first()).toContainText('Featured Games');
    });

    await test.step('Game card titles use h3 under the h2 section', async () => {
      const h3s = page.locator('h3');
      await expect(h3s.first()).toBeVisible();
    });
  });

  test('Heading hierarchy - about page has a single h1 as page title', async ({ page }) => {
    await page.goto('/about');
    await page.waitForSelector('[data-testid="about-section"]', { timeout: 10000 });
    ...
    await test.step('h1 is the About Tailspin Toys heading', async () => {
      await expect(page.getByTestId('about-heading')).toContainText('About Tailspin Toys');
      await expect(page.getByTestId('about-heading')).toHaveRole('heading', { level: 1 });
    });

    await test.step('No h2 or deeper headings skip levels', async () => {
      // The about page body has no sub-sections, so no h2/h3 should be present
      const h2s = page.locator('h2');
      await expect(h2s).toHaveCount(0);
    });
  });

  test('Heading hierarchy - game details page has correct h1 → h2 structure', async ({ page }) => {
    await page.goto('/game/1');
    await page.waitForSelector('[data-testid="game-details"]', { timeout: 10000 });

    await test.step('Page has exactly one h1 (game title)', async () => {
      const h1s = page.locator('h1');
      await expect(h1s).toHaveCount(1);
      await expect(h1s.first()).not.toBeEmpty();
    });

    await test.step('h2 follows h1 for the About this game section', async () => {
      const h2s = page.locator('h2');
      await expect(h2s.first()).toContainText('About this game');
    });
  });
});
